...
flaxdat.subset <- flaxdat.subset[!(flaxdat.subset$Condition.rep =="120HAI_L2.1"),]
dim(flaxdat.subset)
...
```
##Préparation figure pour publication
Pour une publication, il faut une figure montrant la quantité d'acide chicorique, seulement dans les conditions 72HAI, 96HAI_D, et 120HAI_L2. La figure doit montrer un graph avec les quantités par plantule et un graph avec les quantités par gramme.
```{r}
df2.pp <- df2[c(1,2,6),]
df3.pp <- df3[c(1,2,6),]
df2.pp$Condition <- c("72", "96", "120")
df2.pp$Condition = factor(df2.pp$Condition, levels = c("72", "96", "120"))
df3.pp$Condition = factor(df3.pp$Condition, levels = c("72", "96", "120"))

#plot
ggp1 <- ggplot(df2.pp, aes(x = Condition, y = Mass_microg)) +
  geom_point(color = "blue", size = 3) +
  geom_errorbar(aes(ymin = CI.lower_microg, ymax = CI.upper_microg), width = 0.2, color = "black") +
  labs(
    x = "Hours after imbibition",
    y = "Chicoric acid (µg / whole seedling)"
  ) 
 

#plot
ggp2 <- ggplot(df3.pp, aes(x = Condition, y = Mass_microgram.per.g)) +
  geom_point(color = "blue", size = 3) +
  geom_errorbar(aes(ymin = CI.lower, ymax = CI.upper), width = 0.2, color = "black") +
  labs(
    x = "Hours after imbibition",
    y = "Chicoric acid (µg / g)"
  )
 

ggp.all <- (ggp1+ggp2)+
  plot_annotation(title = "Chicoric acid content in flax seedlings")&
  theme(plot.title = element_text(hjust = 0.5))

ggp.all

#create pdf
pdf("chicoricacidcontent.pdf")
ggp.all
dev.off()
